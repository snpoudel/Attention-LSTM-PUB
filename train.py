...
def train_and_evaluate(model, all_data, train_targets, device, folder, seq_len):
    ...
    base_params = [p for n, p in model.named_parameters() if n != 'log_attn_temp']
    optimizer = torch.optim.Adam([
        {'params': base_params, 'lr': hparams['lr']},
        {'params': [model.log_attn_temp], 'lr': 1e-2}, # Higher learning rate for log_attn_temp
    ])
    ...
    for epoch in range(hparams['num_epochs']):
        ...
        if (epoch + 1) % 1 == 0:
            print(f"Epoch {epoch + 1}: Train Loss = {loss.item():.2f}, Log Temp = {model.log_attn_temp.item():.2f}, "
                  f"Temp = {torch.exp(model.log_attn_temp).item():.2f}")
